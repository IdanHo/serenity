...
void xHCIController::take_exclusive_control_from_bios()
{
    ...
    if (extended_capabilities_pointer == 0)
        return;
...
ErrorOr<void> xHCIController::initialize()
{
    ...
    m_runtime_registers.interrupter_registers[0].interrupter_moderation.interrupt_moderation_interval = 500; // max 8000 interrupts/sec
...
void xHCIController::event_handling_thread()
{
    while (!Process::current().is_dying()) {
        m_event_queue.wait_forever(purpose());
