...
static Result<size_t, JS::Value> parse_module(JS::GlobalObject& global_object, JS::Object* buffer)
{
    ...
    WebAssemblyObject::s_compiled_modules.append(make<WebAssemblyObject::CompiledWebAssemblyModule>(move(bytes), module_result.release_value()));
...
JS_DEFINE_NATIVE_FUNCTION(WebAssemblyObject::instantiate)
{
    ...
    auto take_exception_and_reject_if_needed = [&] {
        if (vm.exception()) {
            JS::Value exception = vm.exception()->value();
            promise->reject(exception);
            vm.clear_exception();
            return true;
        }
    ...
    promise->fulfill(vm.heap().allocate<WebAssemblyModuleInstanceObject>(global_object, global_object, s_instantiated_modules.size() - 1));
    return promise;
}
...
WebAssemblyModuleInstanceObject::WebAssemblyModuleInstanceObject(JS::GlobalObject& global_object, size_t index)
    : Object(*global_object.object_prototype())
    , m_index(index)
{
}
...
void WebAssemblyModuleInstanceObject::initialize(JS::GlobalObject& global_object)
{
    Object::initialize(global_object);
    ...
    define_property("exports", m_exports_object);
}

void WebAssemblyModuleInstanceObject::visit_edges(Cell::Visitor& visitor)
{
